    ...
    struct DeviceResourceIface *drsOps, struct SensorDeviceConfig *sensorConfig)
{
    ...
    for (i = 0; i < sensorConfig->ctrlValueNum; i+=5) {
        sensorConfig->ctrlCap[i/5].ctrlId = sensorConfig->ctrlValue[i];
        sensorConfig->ctrlCap[i/5].max = sensorConfig->ctrlValue[i+1];
        sensorConfig->ctrlCap[i/5].min = sensorConfig->ctrlValue[i+2];
        sensorConfig->ctrlCap[i/5].step = sensorConfig->ctrlValue[i+3];
        sensorConfig->ctrlCap[i/5].def = sensorConfig->ctrlValue[i+4];

        HDF_LOGD("%s: get sensor[%{public}d] ctrl cap[%{public}d]: cmd_id=%{public}d, max=%{public}d, "
            "min=%{public}d, step = %{public}d, def = %{public}d",
            __func__, sensorConfig->id, i/5, sensorConfig->ctrlCap[i/5].ctrlId, sensorConfig->ctrlCap[i/5].max,
            sensorConfig->ctrlCap[i/5].min, sensorConfig->ctrlCap[i/5].step, sensorConfig->ctrlCap[i/5].def);
    }
    ...
    struct DeviceResourceIface *drsOps, struct CameraIspConfig *ispConfig)
{
    ...
    CHECK_PARSER_CONFIG_RET(ret, "ctrlValue");
    for (i = 0; i < ispConfig->ctrlValueNum; i+=5) {
        ispConfig->ctrlCap[i/5].ctrlId = ispConfig->ctrlValue[i];
        ispConfig->ctrlCap[i/5].max = ispConfig->ctrlValue[i+1];
        ispConfig->ctrlCap[i/5].min = ispConfig->ctrlValue[i+2];
        ispConfig->ctrlCap[i/5].step = ispConfig->ctrlValue[i+3];
        ispConfig->ctrlCap[i/5].def = ispConfig->ctrlValue[i+4];

        HDF_LOGD("%s: get isp[%{public}d] ctrl cap[%{public}d]: cmd_id=%{public}d, max=%{public}d, "
            "min=%{public}d, step = %{public}d, def = %{public}d",
            __func__, ispConfig->id, i/5, ispConfig->ctrlCap[i/5].ctrlId, ispConfig->ctrlCap[i/5].max,
            ispConfig->ctrlCap[i/5].min, ispConfig->ctrlCap[i/5].step, ispConfig->ctrlCap[i/5].def);
    }
    return HDF_SUCCESS;
}
    ...
    struct DeviceResourceIface *drsOps, struct VcmDeviceConfig *vcmConfig)
{
    ...
    CHECK_PARSER_CONFIG_RET(ret, "ctrlValue");
    for (i = 0; i < vcmConfig->ctrlValueNum; i+=5) {
        vcmConfig->ctrlCap[i/5].ctrlId = vcmConfig->ctrlValue[i];
        vcmConfig->ctrlCap[i/5].max = vcmConfig->ctrlValue[i+1];
        vcmConfig->ctrlCap[i/5].min = vcmConfig->ctrlValue[i+2];
        vcmConfig->ctrlCap[i/5].step = vcmConfig->ctrlValue[i+3];
        vcmConfig->ctrlCap[i/5].def = vcmConfig->ctrlValue[i+4];

        HDF_LOGD("%s: get vcm[%{public}d] ctrl cap[%{public}d]: cmd_id=%{public}d, max=%{public}d, "
            "min=%{public}d, step = %{public}d, def = %{public}d",
            __func__, vcmConfig->id, i/5, vcmConfig->ctrlCap[i/5].ctrlId, vcmConfig->ctrlCap[i/5].max,
            vcmConfig->ctrlCap[i/5].min, vcmConfig->ctrlCap[i/5].step, vcmConfig->ctrlCap[i/5].def);
    }
    ...
    struct DeviceResourceIface *drsOps, struct LensDeviceConfig *lensConfig)
{
    ...
    for (i = 0; i < lensConfig->ctrlValueNum; i+=5) {
        lensConfig->ctrlCap[i/5].ctrlId = lensConfig->ctrlValue[i];
        lensConfig->ctrlCap[i/5].max = lensConfig->ctrlValue[i+1];
        lensConfig->ctrlCap[i/5].min = lensConfig->ctrlValue[i+2];
        lensConfig->ctrlCap[i/5].step = lensConfig->ctrlValue[i+3];
        lensConfig->ctrlCap[i/5].def = lensConfig->ctrlValue[i+4];

        HDF_LOGD("%s: get lens[%{public}d ]ctrl cap[%{public}d]: cmd_id=%{public}d, max=%{public}d, "
            "min=%{public}d, step = %{public}d, def = %{public}d",
            __func__, lensConfig->id, i/5, lensConfig->ctrlCap[i/5].ctrlId, lensConfig->ctrlCap[i/5].max,
            lensConfig->ctrlCap[i/5].min, lensConfig->ctrlCap[i/5].step, lensConfig->ctrlCap[i/5].def);
    }
    ...
    struct DeviceResourceIface *drsOps, struct CameraFlashConfig *flashConfig)
{
    ...
    for (i = 0; i < flashConfig->ctrlValueNum; i+=5) {
        flashConfig->ctrlCap[i/5].ctrlId = flashConfig->ctrlValue[i];
        flashConfig->ctrlCap[i/5].max = flashConfig->ctrlValue[i+1];
        flashConfig->ctrlCap[i/5].min = flashConfig->ctrlValue[i+2];
        flashConfig->ctrlCap[i/5].step = flashConfig->ctrlValue[i+3];
        flashConfig->ctrlCap[i/5].def = flashConfig->ctrlValue[i+4];

        HDF_LOGD("%s: get flash[%{public}d] ctrl cap[%{public}d]: cmd_id=%{public}d, max=%{public}d, "
            "min=%{public}d, step = %{public}d, def = %{public}d",
            __func__, flashConfig->id, i/5, flashConfig->ctrlCap[i/5].ctrlId, flashConfig->ctrlCap[i/5].max,
            flashConfig->ctrlCap[i/5].min, flashConfig->ctrlCap[i/5].step, flashConfig->ctrlCap[i/5].def);
    }
...
struct CameraConfigRoot *HdfCameraGetConfigRoot()
{
