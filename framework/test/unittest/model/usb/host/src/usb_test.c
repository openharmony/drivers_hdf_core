...
{
    int32_t ret = UsbInitHostSdk(&g_session);
    if (ret != HDF_SUCCESS) {
...
{
    int32_t ret = UsbExitHostSdk(g_acm->session);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbExitHostSdk(NULL);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfClaimInterface001(void)
{
    ...
    g_acm->dataIface =
        (struct UsbInterface *)UsbClaimInterface(g_acm->session, g_acm->busNum, g_acm->devAddr, g_acm->interfaceIndex);
...
{
    int32_t ret = UsbReleaseInterface(g_acm->dataIface);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbReleaseInterface(g_acm->intIface);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbReleaseInterface(g_acm->ctrIface);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfSelectInterfaceSetting001(void)
{
    ...
    int32_t ret = UsbSelectInterfaceSetting(g_ecmDataDevHandle, settingIndex, &g_ecmDataIface);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfGetPipe001(void)
{
    ...
    for (i = 0; i <= g_acm->dataIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(NULL, g_acm->dataIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
...
{
    struct UsbPipeInfo p;
    for (uint8_t i = 0; i <= g_acm->dataIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(g_acm->data_devHandle, g_acm->dataIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
        if ((p.pipeDirection == USB_PIPE_DIRECTION_IN) && (p.pipeType == USB_PIPE_TYPE_BULK)) {
...
static int32_t CheckHostSdkIfGetPipe005(void)
{
    ...
    for (i = 0; i <= g_acm->intIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(NULL, g_acm->intIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
...
{
    struct UsbPipeInfo p;
    for (uint8_t i = 0; i <= g_acm->intIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(g_acm->int_devHandle, g_acm->intIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
        if ((p.pipeDirection == USB_PIPE_DIRECTION_IN) && (p.pipeType == USB_PIPE_TYPE_INTERRUPT)) {
...
static int32_t CheckHostSdkIfGetPipe007(void)
{
    ...
    for (i = 0; i <= g_acm->ctrIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(NULL, g_acm->ctrIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
...
static int32_t CheckHostSdkIfGetPipe008(void)
{
    struct UsbPipeInfo p;
    g_acm->interfaceIndex = USB_CTRL_INTERFACE_ID;
    for (uint8_t i = 0; i <= g_acm->ctrIface->info.pipeNum; i++) {
        int32_t ret = UsbGetPipeInfo(g_acm->ctrl_devHandle, g_acm->ctrIface->info.curAltSetting, i, &p);
        if (ret < 0) {
            continue;
        }
        if ((p.pipeDirection == USB_PIPE_DIRECTION_OUT) && (p.pipeType == USB_PIPE_TYPE_CONTROL)) {
...
{
    for (int32_t i = 0; i < ACM_NR; i++) {
        int32_t ret = UsbFreeRequest(g_acm->readReq[i]);
        if (ret != HDF_SUCCESS) {
            HDF_LOGE("%s: error", __func__);
            return HDF_FAILURE;
        }
...
static int32_t CheckHostSdkIfAllocRequest003(void)
{
    ...
    int32_t ret = AcmWriteBufAlloc(g_acm);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    for (int32_t i = 0; i < ACM_NR; i++) {
        int32_t ret = UsbFreeRequest(g_acm->wb[i].request);
        if (ret != HDF_SUCCESS) {
            HDF_LOGE("%s: error", __func__);
            return HDF_FAILURE;
        }
...
{
    int32_t ret = UsbFreeRequest(g_acm->notifyReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFreeRequest006(void)
{
    ...
    int32_t ret = UsbFreeRequest(g_acm->isoReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest001(void)
{
    ...
    int32_t ret = UsbFillRequest(g_acm->isoReq, g_acm->iso_devHandle, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest002(void)
{
    ...
    parmas.dataReq.length = (int)size;
    parmas.dataReq.buffer = (unsigned char *)sendData;
    int32_t ret = UsbFillRequest(NULL, g_acm->iso_devHandle, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest003(void)
{
    ...
    parmas.dataReq.length = (int)size;
    parmas.dataReq.buffer = (unsigned char *)sendData;
    int32_t ret = UsbFillRequest(g_acm->isoReq, NULL, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest004(void)
{
    ...
    parmas.dataReq.length = (int)size;
    parmas.dataReq.buffer = (unsigned char *)sendData;
    int32_t ret = UsbFillRequest(g_acm->isoReq, g_acm->iso_devHandle, NULL);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest005(void)
{
    ...
    parmas.dataReq.length = (int)size;
    parmas.dataReq.buffer = (unsigned char *)sendData;
    int32_t ret = UsbFillRequest(NULL, NULL, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillIsoRequest006(void)
{
    ...
    parmas.dataReq.length = (int)size;
    parmas.dataReq.buffer = (unsigned char *)sendData;
    int32_t ret = UsbFillRequest(g_acm->isoReq, NULL, NULL);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbFreeRequest(g_acm->ctrlReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillRequest001(void)
{
    ...
    int32_t ret = UsbFillRequest(g_acm->readReq[0], g_acm->data_devHandle, &readParmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillRequest002(void)
{
    ...
    g_acm->wb[0].len = (int)size;
    int32_t ret = memcpy_s(g_acm->wb[0].buf, g_acm->writeSize, sendData, size);
    if (ret != EOK) {
    ...
    parmas.userData = (void *)&g_acm->wb[0];
    parmas.dataReq.directon = USB_REQUEST_DIR_TO_DEVICE;
    parmas.dataReq.length = g_acm->wb[0].len;
    parmas.dataReq.buffer = g_acm->wb[0].buf;
    ret = UsbFillRequest(g_acm->wb[0].request, g_acm->data_devHandle, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillRequest003(void)
{
    ...
    intParmas.dataReq.directon =
        (UsbRequestDirection)(((uint32_t)g_acm->intPipe->pipeDirection >> USB_PIPE_DIR_OFFSET) & DIRECTION_MASK);
    intParmas.dataReq.length = (int)g_acm->intSize;
    intParmas.dataReq.buffer = NULL;
    int32_t ret = UsbFillRequest(g_acm->notifyReq, g_acm->int_devHandle, &intParmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillRequest004(void)
{
    ...
    parmas.userData = NULL;
    int32_t ret = UsbFillRequest(g_acm->ctrlReq, g_acm->ctrl_devHandle, &parmas);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestSync(g_acm->readReq[0]);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestSync(g_acm->wb[0].request);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestSync(g_acm->ctrlReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestSync(g_acm->notifyReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfFillRequest005(void)
{
    ...
    for (int32_t i = 0; i < ACM_NR; i++) {
        ...
        readParmas.dataReq.directon =
            (UsbRequestDirection)(((uint32_t)g_acm->dataInPipe->pipeDirection >> USB_DIR_OFFSET) & DIRECTION_MASK);
        readParmas.dataReq.length = (int)g_acm->readSize;
        int32_t ret = UsbFillRequest(g_acm->readReq[i], g_acm->data_devHandle, &readParmas);
        if (ret != HDF_SUCCESS) {
            HDF_LOGE("%s: error", __func__);
            return HDF_FAILURE;
        }
...
static int32_t CheckHostSdkIfFillRequest006(void)
{
    ...
    for (int32_t i = 0; i < ACM_NR; i++) {
        ...
        int32_t ret = memcpy_s(g_acm->wb[i].buf, g_acm->writeSize, sendData, size);
        if (ret != EOK) {
            HDF_LOGE("%s: memcpy_s failed", __func__);
            return HDF_FAILURE;
        }
...
{
    int32_t ret = UsbSubmitRequestAsync(g_acm->readReq[0]);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbCancelRequest(g_acm->readReq[0]);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestAsync(g_acm->wb[0].request);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbCancelRequest(g_acm->wb[0].request);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestAsync(g_acm->notifyReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbCancelRequest(g_acm->notifyReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbSubmitRequestAsync(g_acm->ctrlReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbCancelRequest(g_acm->ctrlReq);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbClearInterfaceHalt(g_acm->data_devHandle, g_acm->dataOutPipe->pipeAddress);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbClearInterfaceHalt(g_acm->int_devHandle, g_acm->intPipe->pipeAddress);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
{
    int32_t ret = UsbClearInterfaceHalt(g_acm->ctrl_devHandle, g_acm->ctrPipe->pipeAddress);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfRemoveInterface001(void)
{
    ...
    int32_t ret = UsbAddOrRemoveInterface(
        g_acm->session, g_acm->busNum, g_acm->devAddr, g_acm->dataIface->info.interfaceIndex, status);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfRemoveInterface002(void)
{
    ...
    OsalSleep(1);
    int32_t ret = UsbAddOrRemoveInterface(
        g_acm->session, g_acm->busNum, g_acm->devAddr, g_acm->intIface->info.interfaceIndex, status);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
...
static int32_t CheckHostSdkIfRemoveInterface003(void)
{
    ...
    int32_t ret = UsbAddOrRemoveInterface(
        g_acm->session, g_acm->busNum, g_acm->devAddr, g_acm->ctrIface->info.interfaceIndex, status);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%s: error", __func__);
        return HDF_FAILURE;
    }
