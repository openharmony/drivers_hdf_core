...
class HeaderParser:
    ...
    def _extract_struct(self, stack):
        ...
        """
        struct_dict = {}
        if stack["declaration_method"] not in ["struct", "class"]:
            return
        if len(stack["methods"]["public"]) != 0:
            return
        if self._has_function_pointer(stack["properties"]["public"]):
            return
        ...
        for mb in stack["properties"]["public"]:
            if "enum_type" in mb:
                struct_dict.get("members").append({
                    "file_name": file_name,
                    "line_number": mb["line_number"],
                    "name": mb["name"],
    ...
    def _extract_interface(self, stack):
        ...
        if stack["declaration_method"] not in ["struct", "class"]:
            return
        # 带函数，或变量中包含函数指针
        if len(stack["methods"]["public"]) <= 0 and not self._has_function_pointer(stack["properties"]["public"]):
            return
