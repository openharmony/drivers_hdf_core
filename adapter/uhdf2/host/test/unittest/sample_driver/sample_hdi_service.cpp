...
static int32_t SampleServiceEndHost(const struct HdfDeviceObject *device)
{
    (void)device;
    ...
    struct DevHostServiceFull *fullService = reinterpret_cast<struct DevHostServiceFull *>(instance);
    struct HdfMessageLooper *looper = &fullService->looper;
    HDF_LOGW("%{public}s: host %{public}d stop", __func__, fullService->super.hostId);
    if ((looper != nullptr) && (looper->Stop != nullptr)) {
        looper->Stop(looper);
    }
    return HDF_SUCCESS;
}
