...
public:
    ...
    virtual int32_t GetDisplaySupportedModes(uint32_t devId,
        std::vector<DisplayModeInfo>& modes) = 0;
    ...
    virtual int32_t SetDisplayClientBuffer(uint32_t devId,
        const BufferHandle& buffer, int32_t fence) = 0;
    ...
    virtual int32_t RegDisplayVBlankCallback(uint32_t devId, VBlankCallback cb,
        void* data) = 0;
    ...
    virtual int32_t SetLayerPosition(uint32_t devId, uint32_t layerId, const IRect& rect) = 0;
